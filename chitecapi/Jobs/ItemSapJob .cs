...
{
    public class ItemSapJob : GenericJob, IJob, IRegisteredObject
    {
        ...
        public void Execute()
        {
            try
            {
                lock (_lock)
                {
                    if (shuttingDown || (!VerifyIsEnable(ConfigurationManager.AppSettings["habilitar_itemsapjob"])))
                    {
                        return;
                    }
                    ...
                    Task<IHttpActionResult> result = sapController.GetSapItems();
