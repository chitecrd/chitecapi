...
{
    public class ConduceJob : GenericJob, IJob, IRegisteredObject
    {
        ...
        public void Execute()
        {
            try
            {
                lock (_lock)
                {
                    if (shuttingDown || (!VerifyIsEnable(ConfigurationManager.AppSettings["habilitar_conducesapjob"])))
                    {
                        return;
                    }
                    ...
                    Task<IHttpActionResult> result = sapController.GetSapConduces();
