...
{
    public class BusinessPartnersSapJob : GenericJob, IJob, IRegisteredObject
    {
        ...
        public void Execute()
        {
            try
            {
                lock (_lock)
                {
                    if (shuttingDown || (!VerifyIsEnable(ConfigurationManager.AppSettings["habilitar_BusinessPartnerssapjob"])))
                    {
                        return;
                    }
                    ...
                    Task<IHttpActionResult> result = sapController.GetSapBusinessPartners();
