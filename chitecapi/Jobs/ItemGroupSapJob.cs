...
{
    public class ItemGroupSapJob : GenericJob, IJob, IRegisteredObject
    {
        ...
        public void Execute()
        {
            try
            {
                lock (_lock)
                {
                    if (shuttingDown || (!VerifyIsEnable(ConfigurationManager.AppSettings["habilitar_itemgroupsapjob"])))
                    {
                        return;
                    }
                    ...
                    Task<IHttpActionResult> result = sapController.GetSapItemsGroup();
